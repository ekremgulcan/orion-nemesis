...
| [`excel-export-button`](./excel-export-button) | Adding an "export to Excel" button/feature to a screen or report, in **any** codebase or tech stack (not Orion-specific) - one shared export function reused by every UI entry point, the full filtered/unpaged result set, in-memory workbook generation, dated filenames, and consistent button/loading/toast UX, with concrete backend recipes (Java/Apache POI, Node.js/ExcelJS, Python/openpyxl, .NET/ClosedXML) and a frontend blob-download recipe. Generalized from the exact pattern first validated in this project's own "Rapor Olustur"/Bildirim Izleme export feature. |
