...
export function HisseKotasyonPage() {
  const { setTitle } = useOutletContext<PageTitleContext>()
  useEffect(() => {
    setTitle("Hisse Kotasyon Izleme")
  }, [setTitle])
  ...
  const { data: instruments = [], isLoading } = useQuery({
    queryKey: ["instruments", "HISSE", query],
    queryFn: () => fetchInstruments("HISSE", query || undefined),
  })
  ...
  return (
    <div className="flex min-h-0 flex-1">
      {/* Middle column: search + static instrument list */}
      <div className="flex min-w-0 flex-1 flex-col border-r border-border">
        <div className="flex flex-col gap-2 border-b border-border px-6 py-4">
          <p className="text-xs text-foreground-faint">
            Canli fiyat akisi Faz 4+ kapsaminda eklenecektir. Asagida statik enstruman listesi
            gosterilmektedir.
          </p>
          <Input
      ...
      <aside className="hidden w-96 shrink-0 flex-col bg-surface lg:flex">
        {!selected && (
          <div className="flex flex-1 flex-col items-center justify-center gap-2 px-6 text-center">
            ...
            <p className="text-sm text-foreground-muted">Bir hisse secin</p>
          </div>
        )}
