...
export function MusteriBildirimTercihleriPage() {
  ...
  const [searchedMusteriNo, setSearchedMusteriNo] = useState<string | null>(null)
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData(["musteri-bildirim-tercihleri", searchedMusteriNo], data)
      toast.success("Bildirim tercihleri kaydedildi.")
    },
    onError: (error) => {
      toast.error(extractErrorMessage(error, "Bildirim tercihleri kaydedilirken hata olustu"))
    },
  })
  ...
  function updateRow(notificationTypeId: number, patch: Partial<BildirimTercihiDto>) {
    setRows((prev) => prev.map((row) => (row.notificationTypeId === notificationTypeId ? { ...row, ...patch } : row)))
  }

  const found = !!query.data
  const searchError = query.isError ? extractErrorMessage(query.error, "Musteri bulunamadi") : null

  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-col gap-1 border-b border-border px-6 py-4">
        <p className="text-sm font-medium text-foreground">Musteri Bildirim Tercihleri Ekrani</p>
        <p className="text-xs text-foreground-faint">
          Musteri No girerek bir musterinin bildirim tipi bazinda kanal tercihlerini goruntuleyin ve guncelleyin.
        </p>
      </div>
          ...
          {found && query.data && (
            <>
              <CustomerSummaryCard
                ...
                extra={[{ label: "Son Guncelleme", value: formatSonGuncelleme(query.data.sonGuncelleme) }]}
              />

              <div className="rounded-lg border border-border bg-surface">
                <div className="flex items-center justify-between rounded-t-lg bg-info px-4 py-2.5 text-sm font-semibold text-white">
                  <span>Bildirim Tercihleri (Bildirim Tipi Bazinda)</span>
                </div>
                <div className="p-4">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-center">Push Bildirim</TableHead>
                      ...
                      {rows.map((row) => (
                        <TableRow key={row.notificationTypeId}>
                          ...
                          <ToggleCell
                            row={row}
                            field="pushAcik"
                            onChange={(checked) => updateRow(row.notificationTypeId, { pushAcik: checked })}
                          />
                          <ToggleCell
                            row={row}
                            field="smsAcik"
                            onChange={(checked) => updateRow(row.notificationTypeId, { smsAcik: checked })}
                          />
                          <ToggleCell
                            row={row}
                            field="epostaAcik"
                            onChange={(checked) => updateRow(row.notificationTypeId, { epostaAcik: checked })}
                          />
                        </TableRow>
                      ))}
                    </TableBody>
  ...
  return (
    <TableCell>
      <div className="flex items-center justify-center gap-2">
        ...
        <Switch checked={checked} onCheckedChange={onChange} disabled={row.zorunlu} />
        <span className="text-xs text-foreground-muted">{checked ? "Acik" : "Kapali"}</span>
      </div>
    </TableCell>
  )
}
